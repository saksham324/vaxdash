  ...
  totalsDoc
    ...
    .then((doc) => {
      if (doc.exists) {
        ...
        var firstTotal = parseInt(firstOn) + parseInt(firstOff); 
        var secondTotal = parseInt(secondOn) + parseInt(secondOff); 
        document.getElementById("firstTotal").innerHTML = String(firstTotal);
        document.getElementById("secondTotal").innerHTML = String(secondTotal);
      } else {
    ...
    .then((doc) => {
      if (doc.exists && !doc.data().updated) {
        if (doc.data().dose == "first") {
          if (doc.data().vaccineMf == "J&J") {
            doc.ref.update({dose: "second"}); 
          }
        if (doc.data().dose == "first") {
...
function updateTotalsByDate() { 
  db.collection("vaccineEntries").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
      console.log("function called"); 
      if (doc.data().dose == "first") {
        var dateParts = doc.data().doseDate.split('-'); 
        var date = new Date(dateParts[0], dateParts[1] - 1, dateParts[2]); 
        var today = new Date(); 
        var newDate = new Date(date.getFullYear(), date.getMonth(), date.getDay() + 30); 
        if (today > newDate) {
            console.log("dose changed to second"); 
            doc.ref.update({dose: "second"}); 
          } 
          }
    });
